...
const postPoll = async (eq, res) => {
  // extract poll information from req
  const{ownerId, title, descritption, optinos} = req.body;
...
const postVote = async (req, res) => {
  ...
  if (!pollId || !optionId)
    return res.status(400).json({error: 'Invalid request'}); // update for 400 Error
  ...
  res.status(200).jscon(updatedPoll);
};
